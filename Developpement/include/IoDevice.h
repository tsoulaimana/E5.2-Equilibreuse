...
public:
    ...
    virtual UlError checkScanState(bool* scanDone = NULL) const { Q_UNUSED(scanDone); return ERR_BAD_DEV_TYPE;}

    virtual void updateScanParam(int param)  {Q_UNUSED(param);};
    virtual void processScanData(void* transfer) {Q_UNUSED(transfer);};
    virtual unsigned int processScanData(void* transfer, unsigned int stageSize) { Q_UNUSED(transfer); Q_UNUSED(stageSize); return 0;}
